...
::banner{type=info}
Always check for pending database migrations after installing or updating xbat and run them as soon as possible to prevent any issues with the databases. See [Database Migrations](/docs/admin/setup/database#migrations) for more information.
::
