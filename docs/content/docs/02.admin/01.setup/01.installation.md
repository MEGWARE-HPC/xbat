...
WARNING: [restapi] 'host' must be replaced with a valid hostname or IP that is accessible for the xbatd
WARNING: [restapi] 'client_secret' must be replaced with the secret configured in MongoDB for the xbatd user

=== ClickHouse Configuration ===
✓ password is set
✓ daemon_password is set
✓ host is set
✓ port is set
✓ user is set
✓ daemon_user is set
✓ database is set
WARNING: [clickhouse] 'host' has been changed from docker-compose default - only change this value if you are deploying with --no-db
WARNING: [clickhouse] 'port' has been changed from docker-compose default port - only change this value if you are deploying with --no-db
...
✓ user is set
✓ password is set
WARNING: [pgbouncer] 'host' must not be changed (default 'xbat-pgbouncer')
✓ pgbouncer database matches ClickHouse database

=== Valkey Configuration ===
✓ host is set
✓ port is set
✓ database is set
WARNING: [valkey] 'host' has been changed from docker-compose default - ensure this is correct for your setup

=== Authentication Configuration ===
✓ provider is set
✓ LDAP address is set
✓ LDAP basedn is set
✓ LDAP user_identifier is set

=== Demo Configuration ===
✓ enabled is set
✓ demo user is set
✓ demo password is set
WARNING: [demo] Demo mode is enabled - should be disabled in production!
