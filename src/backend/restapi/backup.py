...
async def process_table(csvs_path, jobId_map):
    ...
    if table_dict:
        jobId_list = []
        try:
            with open(table_dict["likwid_l2_bw"],
                      'r',
                      newline='',
                      encoding='utf-8') as likwid_l2_bw_file:
                reader = csv.DictReader(likwid_l2_bw_file)
        ...
        data_count = await questdb.execute_query(
            f"SELECT COUNT(*) AS count FROM likwid_l2_bw WHERE jobId IN {jobIds};"
        )
        if data_count:
